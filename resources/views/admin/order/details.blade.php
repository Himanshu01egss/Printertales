...
<div class="col-sm-4">
<div class="card">
<div class="card-header"><h3 class="title">Customer Details</h3></div>
<div class="card-body">
...
<div class="col-sm-4">
  ...
  <div class="card">
<div class="card-header"><h3 class="title">Payment Info</h3></div>
<div class="card-body">
  <table class="table1">
    <tr>
      <th>Status :- </th>
...
?>
    ...
    <tr>
      ...
      <td>
        ...
        ?>
       <a href="{{url('admin/order/invoice_print')}}/{{base64_encode($ord->order_id)}}" class="text-info"> <i class="fa fa-download"></i></a></td>
    </tr>
  </table>
<?php
...
<tbody>
 ...
 for($j=0;$j<count($arr);$j++){
  ...
  foreach($cart_detail as $crt){
    ...
    foreach($pro_d as $p){
      $pr_img=$p->image;
      $pr_business=$p->business_id;
    }
    ?>
<tr>
<td style="max-width:100px;"><img src="{{url('public')}}/{{$pr_img}}" alt="" class="avatar-sm" style="height: 50px;"></td>

<td style="max-width:500px;">
<div><h5 class="text-truncate font-size-14" style="margin-bottom: -7px;"><?php echo $crt->product_name; ?>
</h5>
<?php

$get_prodq=DB::table("products")->where("id",$crt->product_id)->get();
foreach ($get_prodq as $get_prod) {
if($get_prod->business_id==null){
  $g_business_id=0;
}else{
  $g_business_id=$get_prod->business_id;
}
}
// $get_prod=DB::table("products")->where("id",$g_business_id)->first();
$get_business=DB::table("business")->where("user_id",$g_business_id)->get();
foreach ($get_business as $business_d) {
echo "<small class='text-info' style='margin-top:-20px;'>Business : ".$business_d->business_name."</small>";
}
?>
@if (isset(json_decode($crt->attributes,true)['size']))
  <p class="text-muted mb-0">Size : {{ json_decode($crt->attributes,true)['size'] }}</p>
@endif
<p class="text-muted mb-0">Rs {{$crt->price}} x {{$crt->qty}}</p>
</div>
</td>
...
<tr><td colspan="2"><h6 class="m-0 text-right">Shipping:</h6></td>
