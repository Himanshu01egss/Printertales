...
<div class="wrapper">
  ...
  @show
<!-- Modal -->
<div class="modal fade" id="order_placed" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-labelledby="order_placed" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">      
      <div class="modal-body">
        <h4 class="text-center" id="msg_ord">A New Order Has been Received</h4>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close" style="position: absolute;right: -9px;
        top: -10px;padding: 1px 6px;border: 1px #ef0e0e solid;border-radius: 13px;background: red;color: #fff;        opacity: 1;">
...
$.get('{{url("/get_subcategory")}}',{ category_id:category_id}, function(data){
$("#subcat").html(data);
});
})
...
function change_status(id,value){
  ...
  $.ajax({
    type:'POST',
    url:'{{url("admin/product/category_status")}}',
    headers: {
...
function change_status_brand(id,value){
  ...
  $.ajax({
    type:'POST',
    url:'{{url("admin/product/category_status_brand")}}',
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
    data:{id:id,status:value},
...
function change_status_sub(id,value){
  ...
  $.ajax({
    type:'POST',
    url:'{{url("admin/product/category_status_sub")}}',
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
    data:{id:id,status:value},
...
console.log(res);
 ...
 function coupon_status(id,value){
  ...
  $.ajax({
    type:'POST',
    url:'{{url("admin/coupon/update_status")}}',
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
    data:{id:id,status:value},
...
function change_popular(id,value){
  ...
  $.ajax({
    type:'POST',
    url:'{{url("admin/category/change_popular")}}',
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
    data:{id:id,status:value},
...
console.log(res);
  ...
  $.ajax({
    type:'POST',
    url:'{{url("admin/customer/update_status")}}',
    headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
    data:{id:id,status:value},
...
function edit_subcat(id){
  var id=id
   $.get("{{url('admin/produtct/editsubcategory')}}",{id:id}, function(res){
...
$.ajax({
  type:'POST',
  url:'{{url("admin/product/business-product/update_status")}}',
  headers:{
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
...
$.get("{{('admin/get_data')}}", function(res) {
  var obj = JSON.parse(res);
  var ctx = document.getElementById('barChart').getContext('2d');
var chart = new Chart(ctx, {
    type: 'bar',
...
$.get("{{('admin/day_wise')}}", function(res) {
  function days(month,year){
         return new Date(year, month, 0).getDate();
      }
      var arr=[];
...
function today_wise() {
$.get("{{('admin/hour_wise')}}", function(res) {
  function days(month,year){
         return new Date(year, month, 0).getDate();
      }
      var arr=[];
...
<script>
    $(document).ready(function(){
        ...
        function updateToDatabase(idString){
           ...
           $.ajax({
              url:"{{url('admin/product/update_image_order')}}",
              method:'POST',
              data:{ids:idString},
              success:function(res){
                // console.log(res)
